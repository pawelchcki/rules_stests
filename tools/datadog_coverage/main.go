...
// verifyCompiledArtifacts keeps retained evidence tied to the bytecode that
// was selected by the driver.  validate intentionally operates on decoded
// values for its focused proof-set tests; the CLI must additionally prove the
// files alongside each retained manifest still match those values.
func verifyCompiledArtifacts(manifestPath string, m manifest) error {
	for scenario, artifact := range m.CompiledValidators {
		if artifact.Path == "" || filepath.IsAbs(artifact.Path) || !filepath.IsLocal(artifact.Path) ||
			!digestRE.MatchString(artifact.SourceSHA256) || !digestRE.MatchString(artifact.CompilerSHA256) || !digestRE.MatchString(artifact.BytecodeSHA256) {
			return fmt.Errorf("invalid compiled validator artifact %s/%s", m.Profile, scenario)
		}
		bytecode, err := os.ReadFile(filepath.Join(filepath.Dir(manifestPath), artifact.Path))
		if err != nil {
			return fmt.Errorf("read compiled validator artifact %s/%s: %w", m.Profile, scenario, err)
		}
		if len(bytecode) == 0 || fmt.Sprintf("%x", sha256.Sum256(bytecode)) != artifact.BytecodeSHA256 {
			return fmt.Errorf("compiled validator bytecode binding mismatch %s/%s", m.Profile, scenario)
		}
	}
	return nil
}
...
func main() {
	...
	for i, p := range manifestPaths {
		...
		if err := verifyCompiledArtifacts(p, manifests[i]); err != nil {
			fatal(err)
		}
	}
