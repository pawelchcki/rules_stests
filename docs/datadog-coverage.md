...
Local Ruby/Gin payloads are built reproducibly with `tools/build_datadog_fixtures.sh`; this workflow verifies the reviewed rootfs payload digests and does not publish images. The OCI envelope may vary with the container tool's compression and history serialization, which do not affect the extracted rootfs the harness executes. Application Gemfiles and lockfiles remain untouched. Bootstrap checks cover frozen Bundler, repeated/early activation, incompatible or absent dependencies, ABI mismatch, and missing native extensions.
