...
Local Ruby/Gin payloads are built reproducibly with `tools/build_datadog_fixtures.sh`; this workflow verifies reviewed OCI digests and does not publish images. Application Gemfiles and lockfiles remain untouched. Bootstrap checks cover frozen Bundler, repeated/early activation, incompatible or absent dependencies, ABI mismatch, and missing native extensions.
