...
type ddNativeSpan struct {
	...
	MetaStruct map[string]json.RawMessage `json:"meta_struct,omitempty"`
	SpanLinks  []json.RawMessage          `json:"span_links,omitempty"`
...
func knownGoManualDropRule(c ddCase, baseline, spans []ddNativeSpan) bool {
	keep := 2
	c.Priority = &keep
	if validateDatadogCase(c, baseline, spans) != nil || validateDatadogProbes(c, spans, nil) != nil {
		return false
	}
	children := 0
	for _, s := range spans {
		if s.Meta["probe.request_id"] != "" && (s.Meta["_dd.p.dm"] != "-3" || s.Metrics["_dd.rule_psr"] != 1) {
			return false
		}
		if s.Name == "probe.parent" || s.Name == "probe.child" {
			if s.Metrics["_sampling_priority_v1"] != -1 {
				return false
			}
			children++
		}
