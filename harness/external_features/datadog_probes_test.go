...
func probeFixture(kind string, partial bool) ([]ddNativeSpan, []byte) {
	...
	for i, root := range roots {
		...
		makeSpan := func(id, parent uint64, name, resource string) ddNativeSpan {
			return ddNativeSpan{TraceID: root.TraceID, SpanID: id, ParentID: parent, Name: name, Resource: resource, Service: root.Service, Meta: map[string]string{"_dd.p.tid": "1234567890abcdef"}, Metrics: map[string]float64{}}
		}
		switch kind {
