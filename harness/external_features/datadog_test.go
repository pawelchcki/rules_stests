...
func ddBaselineFixture() []ddNativeSpan {
	...
	for i := 1; i <= 4; i++ {
		result = append(result, ddNativeSpan{TraceID: uint64(i), SpanID: uint64(i), Name: "aiohttp.request", Service: "external-probe", Type: "web", Meta: map[string]string{"probe.request_id": fmt.Sprint(i), "env": "test", "version": "1", "http.method": "GET", "http.status_code": "200", "probe.header": "visible", "http.useragent": "datadog-external-probe"}, Metrics: map[string]float64{"_sampling_priority_v1": 2}})
	}
	return result
}
