...
func ddProbeCases() []ddCase {
	...
	result := []ddCase{
		...
		{Name: "manual-keep", Kind: "keep", Priority: &keep, Env: map[string]string{"DD_TRACE_SAMPLING_RULES": "[]"}, Source: "test_sampling_manual.py"},
		{Name: "manual-drop", Kind: "drop", Priority: &drop, Env: map[string]string{"DD_TRACE_SAMPLING_RULES": "[]"}, Source: "test_sampling_manual.py"},
		{Name: "manual-drop-rule", Kind: "drop", Priority: &drop, Source: "test_sampling_manual.py"},
...
func validateDatadogProbes(c ddCase, spans []ddNativeSpan, early []byte) error {
	...
	for _, s := range spans {
		if s.TraceID == 0 || s.SpanID == 0 || s.Start <= 0 || s.Duration <= 0 {
			return fmt.Errorf("invalid probe identity or completion timing")
		}
	...
	parents, children, after := 0, 0, 0
	for _, root := range servers {
		perParent, perChild, perAfter, perClient, perEcho := 0, 0, 0, 0, 0
		var parent ddNativeSpan
		for _, s := range spans {
			if !ancestor(s, root) {
				if s.TraceID == root.TraceID && s.Meta["_dd.p.tid"] == root.Meta["_dd.p.tid"] && s.SpanID != root.SpanID {
					return fmt.Errorf("orphan span in probe request graph")
				}
				continue
			}
			switch s.Name {
		...
		if c.Partial {
			...
			for _, record := range records {
				for _, chunk := range record.Payload.Traces {
					...
					threshold, err := strconv.Atoi(c.Env["DD_TRACE_PARTIAL_FLUSH_MIN_SPANS"])
					if err == nil && len(chunk) < threshold {
						return fmt.Errorf("partial chunk exported below configured threshold")
					}
					position := 0
					if c.ChunkMetadataPosition == "last" {
					...
					for i, later := range chunk {
						if i != position && later.Meta["_dd.p.dm"] != "" {
							return fmt.Errorf("chunk propagation metadata repeated outside designated span")
						}
					}
				}
			}
