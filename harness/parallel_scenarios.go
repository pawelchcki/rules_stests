		...
		for _, chunk := range record.Payload.Traces {
			...
			for _, span := range chunk {
				if value := span.Meta["_dd.p.tid"]; value != "" {
					...
					if high != "0000000000000000" && high != value {
			...
			for _, span := range chunk {
				key := stressIdentity{span.TraceID, high, span.SpanID}
...
func validateStress(capture []byte, ledger []requestLedger, service string) error {
	...
	for key, span := range spans {
		...
		servers[id] = key
		if span.Service != service || span.Meta["http.method"] != request.Method || span.Meta["http.status_code"] != strconv.Itoa(request.Status) {
	...
	for key, span := range spans {
		...
		if span.Type == "sql" {
			...
			if strings.Count(span.Resource, "rules_stests_request=") != 1 || !strings.Contains(span.Resource, marker) {
