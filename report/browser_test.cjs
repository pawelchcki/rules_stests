...
(async () => {
  ...
  assert.equal(await page.inputValue('#proof-status'),'verified');
  assert.match(await page.locator('#feature-matrix').innerText(),/Implemented \(passed test\)/);
  assert.match(await page.locator('#capability-summary').innerText(),/Implemented with a passing test/);
  assert.ok(await page.locator('#category-overview .category-card').count()>3);
  assert.ok(await page.locator('#feature-matrix .topic-row').count()>0);
  ...
  await page.selectOption('#proof-status','not-demonstrated');
  assert.doesNotMatch(await page.locator('#feature-matrix .capability-cell').first().innerText(),/Implemented \(passed test\)/);
  assert.match(await page.locator('#feature-matrix').innerText(),/Checks, no passing result|No checks/);
  await page.selectOption('#category','Metrics');
  assert.ok(await page.locator('#topic option').count()>2);
  await page.selectOption('#proof-status','verified');
  await page.selectOption('#language','python');
