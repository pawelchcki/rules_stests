...
func TestLoadReportManifestDerivesLegacyInputs(t *testing.T) {
	...
	manifest := `[
  {"id":"go-profile","spec":"corpus/realworld/profile/go-profile.scm","plan":"bazel-out/go.proof-plan.json","shapes":{"tags":"corpus/realworld/shape/go-profile/tags.scm","articles":"corpus/realworld/shape/go-profile/articles.scm"}},
  {"id":"python-profile","spec":"corpus/realworld/profile/python-profile.scm","plan":"bazel-out/python.proof-plan.json","shapes":{"tags":"corpus/realworld/shape/python-profile/tags.scm"}}
]`
	if err := os.WriteFile(path, []byte(manifest), 0o644); err != nil {
		t.Fatal(err)
	}
	profiles, scenarios, plans, shapes, err := loadReportManifest(path, "https://example.test/repo/")
	if err != nil {
		t.Fatal(err)
	}
	...
	if got := strings.Join(plans, "\n"); !strings.Contains(got, "go-profile,bazel-out/go.proof-plan.json,https://example.test/repo/corpus/realworld/profile/go-profile.scm") {
		t.Fatalf("plans = %q", got)
	}
	if got := strings.Join(shapes, "\n"); !strings.Contains(got, "go-profile,articles,corpus/realworld/shape/go-profile/articles.scm,https://example.test/repo/corpus/realworld/shape/go-profile/articles.scm") {
		t.Fatalf("shapes = %q", got)
	}
	if _, _, _, _, err := loadReportManifest(path, ""); err == nil || !strings.Contains(err.Error(), "source-root") {
