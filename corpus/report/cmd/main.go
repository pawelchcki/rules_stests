...
func main() {
	var matrixPath, metadataPath, outputPath, profileList, scenarioList, repositoryRevision string
	...
	flag.StringVar(&repositoryRevision, "repository-revision", "main", "repository commit used for evidence links")
	...
	flag.Parse()
	if err := run(matrixPath, metadataPath, outputPath, profileList, scenarioList, repositoryRevision, manifestPaths, goldenSpecs, evidenceRefs); err != nil {
		fmt.Fprintln(os.Stderr, "feature parity report:", err)
		os.Exit(1)
	}
}

func run(matrixPath, metadataPath, outputPath, profileList, scenarioList, repositoryRevision string, manifestPaths, goldenSpecs, evidenceRefs []string) error {
	...
	if err := report.PinRepositoryRevision(manifests, goldens, repositoryRevision); err != nil {
		return err
	}
