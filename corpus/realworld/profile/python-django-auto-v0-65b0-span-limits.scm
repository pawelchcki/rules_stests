(define-library (realworld profile python-django-auto-v0-65b0-span-limits)
  (export profile)
  (import (scheme base)
          (otel profile)
          (otel declarations)
...
(define profile
  (realworld-profile
    (id 'python-django-auto-v0-65b0-span-limits)
    (display-name "Python Django (auto 0.65b0, span attribute limit)")
    (language 'python)
    (framework "Django")
    ...
    (all (observed span/attribute-limit))
    (all (observed environment-variables/otel-span-attribute-count-limit))
    (all (corroborated (sources python-trace-api) sampling/span-limits))
    (all (corroborated (sources python-trace-api) sampling/attribute-limits))
