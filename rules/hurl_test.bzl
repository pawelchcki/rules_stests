...
def realworld_parallel_hurl_test(name, service, profile, sink, cases, tags = []):
    ...
    service_test(
        ...
        args = ["--parallel-scenarios", "--service-suffix=" + str(native.package_relative_label(service)),
                "--hurl-rootfs=" + _rootpath(_HURL_ROOTFS),
                "--telemetry-profile-manifest=" + _rootpath(profile),
                "--telemetry-sink-suffix=" + str(native.package_relative_label(sink))] + [_rootpath(spec) for spec in specs],
